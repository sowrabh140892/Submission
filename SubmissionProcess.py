...
PIXIT_TYPE=str(line[10])
PIXIT_ENTITY_ID=str(line[11])
PIXIT_ENTITY_TYPE=str(line[12])
PIXIT_ATTRIBUTE_NAME=str(line[13])
PIXIT_ATTRIBUTE_VALUE=str(line[14])
...
def main():
    ...
    if count == array_size:
                ...
                msg_attr['entityId'] = {"DataType": "Number", "StringValue": PIXIT_ENTITY_ID}
                msg_attr['entityType'] = {"DataType": "String", "StringValue": PIXIT_ENTITY_TYPE}
                msg_attr['attributeName'] = {"DataType": "String", "StringValue": PIXIT_ATTRIBUTE_NAME}
                response = send_message_to_sns(topic_arn=UAP_SNS_TOPIC, message_body=msg_body, message_attrs=msg_attr)
                print(response)
